...
python defense.py --device 2 --workers 0 --batch-size 1 --data data/custom_argoverse.yaml --weights models/pretrained_model_NEW/argoverse_cl4_best.pt --model_path [path]

# 2. Adversarial Patch Attack Defense
python defense.py --device 2 --workers 0 --batch-size 1 --data data/custom_argoverse.yaml --weights models/pretrained_model_NEW/argoverse_cl4_best.pt --od_shield
...
def run(
    ...
    if training:  # called by train.py
        ...
        patch_dirs = glob(os.path.join(patch_path, '*.png'))
        ...
        defense = ReconstructiveSubNetwork(in_channels=3, out_channels=3)
        defense.load_state_dict(torch.load(model_path, map_location=device))
        defense.cuda()
        defense.eval()
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        targets_tmp = targets[:, 1:]
        targets_tmp = targets_tmp.unsqueeze(dim=0)

        im = im.to(device)
        targets_tmp = targets_tmp.to(device)

        # Attach adversarial patch to raw image
        patch = random.choice(patch_dirs)
        adv_patch_cpu = read_image(path=patch)
        adv_patch = adv_patch_cpu.to(device)
        adv_batch_t = patch_transformer(adv_patch, targets_tmp,
                                        model_in_sz=[640, 640],
                                        use_mul_add_gau="all",
                                        do_transforms=True,
                                        do_rotate=True,
                                        rand_loc=True) 
        im_raw = im.clone()
        im = patch_applier(im, adv_batch_t)

        # Defense
        if od_shield:
            im, _ = defense(im, im_raw) # patched_x, raw_x
